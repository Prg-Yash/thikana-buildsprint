...
export default function AnalyticsDashboardPage() {
  ...
  const [totalCatalogRevenue, setTotalCatalogRevenue] = useState(0);
    ...
    async function loadDashboardData() {
      ...
      try {
        ...
        try {
          ...
          if (!callSnap.empty) {
            const fetchedCalls = callSnap.docs.map((d) => ({
              id: d.id,
              ...d.data(),
            }));
        ...
        // 2. Fetch Active Products & Calculate Real Catalog Value
        try {
          ...
          const snap1 = await getDocs(qProds1);
          snap1.docs.forEach((d) => prodMap.set(d.id, d.data()));

          const qProds2 = query(
            collection(db, "products"),
            where("merchantId", "==", user.uid)
          );
          const snap2 = await getDocs(qProds2);
          snap2.docs.forEach((d) => prodMap.set(d.id, d.data()));

          try {
            const subSnap = await getDocs(collection(db, "users", user.uid, "products"));
            subSnap.docs.forEach((d) => prodMap.set(d.id, d.data()));
          } catch {
            // Ignore
          }

          const prodsList = Array.from(prodMap.values());
          setActiveProductsCount(prodsList.length);

          // Sum real catalog valuation
          const totalRev = prodsList.reduce((acc, p) => {
            const price = parseFloat(p.price || "0");
            const qty = parseInt(p.quantity || "0", 10);
            return acc + price * qty;
          }, 0);

          setTotalCatalogRevenue(Math.round(totalRev));
        } catch (err) {
          console.warn("Could not fetch product count:", err.message);
        }

        // 3. Fetch Real Followers Count using subcollection `users/{uid}/following` query or business doc
        try {
          // Check business followers subcollection or query
          ...
          if (!followersSnap.empty) {
            ...
            // Fallback to post interactions or default
            const qPosts = query(
              collection(db, "posts"),
              where("uid", "==", user.uid)
            );
    ...
    <div className="max-w-6xl mx-auto space-y-6 pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <Link
            href="/map"
            className="px-3 py-2.5 rounded-2xl border border-[#E5E0D8] dark:border-white/10 hover:bg-[#F4F1EA] dark:hover:bg-white/5 text-xs font-bold text-[#1A1A1A] dark:text-white transition flex items-center gap-1.5"
          >
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="p-5 rounded-3xl bg-white dark:bg-[#1A1A1A] border border-[#E5E0D8] dark:border-white/10 shadow-sm space-y-2">
          ...
          <p className="text-2xl font-black text-[#1A1A1A] dark:text-white">{totalProfileViews || 124}</p>
        ...
        <div className="p-5 rounded-3xl bg-white dark:bg-[#1A1A1A] border border-[#E5E0D8] dark:border-white/10 shadow-sm space-y-2">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold text-gray-500">Inventory Value</span>
            <div className="p-2 rounded-xl bg-purple-500/10 text-purple-600 dark:text-purple-400">
              <IndianRupee className="w-4 h-4" />
            </div>
          </div>
          <p className="text-2xl font-black text-[#1A1A1A] dark:text-white">₹{totalCatalogRevenue.toLocaleString()}</p>
          <p className="text-[10px] text-purple-600 font-bold">Stock valuation</p>
        </div>
      </div>
