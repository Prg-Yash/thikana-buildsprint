...
export default function InventoryProductsPage() {
  ...
  useEffect(() => {
    async function loadProductsData() {
      ...
      try {
        ...
        // 1. Query top-level `products` collection by `userId` or `merchantId`
        try {
          const qTop1 = query(collection(db, "products"), where("userId", "==", user.uid));
          const snap1 = await getDocs(qTop1);
          snap1.docs.forEach((d) => prodMap.set(d.id, { id: d.id, ...d.data() }));

          const qTop2 = query(collection(db, "products"), where("merchantId", "==", user.uid));
          const snap2 = await getDocs(qTop2);
          snap2.docs.forEach((d) => prodMap.set(d.id, { id: d.id, ...d.data() }));
        } catch (e) {
          console.warn("Error querying top-level products:", e.message);
        }

        // 2. Query user-scoped `users/{userId}/products` subcollection for compatibility
        try {
          const subSnap = await getDocs(collection(db, "users", user.uid, "products"));
          subSnap.docs.forEach((d) => prodMap.set(d.id, { id: d.id, ...d.data() }));
        } catch {
          // Ignore
        }
  ...
  const handleUpdateStock = async (prodId) => {
    ...
    try {
      // Sync update across both paths
      try {
        await updateDoc(doc(db, "products", prodId), { quantity: newQty });
      } catch {
        // Ignore
      }
      try {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
            Manage product catalog, stock availability, pricing, and GST tax codes.
          </p>
        </div>
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 bg-white dark:bg-[#1A1A1A] p-4 rounded-3xl border border-[#E5E0D8] dark:border-white/10 shadow-sm">
        <div className="relative flex-1 w-full sm:w-auto">
          ...
          <input
            ...
            className="w-full bg-[#F7F6F3] dark:bg-[#262626] border border-transparent focus:border-[#1A1A1A] dark:focus:border-white/20 rounded-2xl py-2 pl-10 pr-4 text-xs font-medium outline-none"
          />
        </div>
              ...
              >
                ...
                <div className="pt-3 border-t border-[#E5E0D8] dark:border-white/10 space-y-3">
                  ...
                  <div className="flex items-center justify-between pt-1">
                    <span className="text-[10px] font-medium text-gray-400">HSN: {prod.hsn || "N/A"}</span>
